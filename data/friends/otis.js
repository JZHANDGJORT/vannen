const otis = {

    id: "otis01",

    name: "Otis",

    // Bild som visas på Vännen-startsidan
    image: "images/Vannen/otis-home.PNG",

    // Bild som visas inne i Otis värld
worldImage: "images/Otis/otis-world.PNG",

// Otis som transparent figur
characterImage: "images/Otis/otis-stone-character.PNG",

characterBlinkImage: "images/Otis/otis-stone-blink-character.PNG",
    
    cardText:
        "En lekfull och nyfiken utter som finns där för små äventyr och lugna stunder.",

    animal: "utter",

    home: {
        name: "Vid vattnet",
        theme: "water"
    },


    personality: [
        "lekfull",
        "nyfiken",
        "omtänksam"
    ],

    introduction: 
        "Hej! Jag heter Otis. Jag tycker om att flyta omkring, upptäcka nya saker och samla på fina små skatter som jag hittar längs vägen.",


    likes: [
        "vatten",
        "stenar",
        "små skatter",
        "att samla och plocka med saker",
        "att flyta omkring",
        "att leka",
        "att surfa på magen i snön"
    ],


    description:
        "Otis är en lekfull och nyfiken utter som tycker om både äventyr och lugna stunder. Han vill upptäcka världen tillsammans med dig och påminna om att små saker kan göra en dag fin.",

subtitle:
    "Din vän vid vattnet",
    
    about:
`Jag är en liten utter som tycker väldigt mycket om att upptäcka världen runt omkring mig. 🌊

Jag älskar att flyta omkring i lugna vatten, leta efter små skatter och hitta fina stenar som någon kanske har tappat bort.

Jag tycker om både busiga äventyr och stilla stunder där vi bara kan ta det lugnt tillsammans. Ibland behöver man inte göra något stort för att en dag ska kännas fin.

Jag hoppas att jag kan få vara din lilla vän att komma tillbaka till när du vill prata, leka eller bara ha en lugn stund. 💚`,
    
    purpose:
        "Jag hoppas att vi kan ta hand om varandra och göra dagarna lite mjukare.",


    voice: {
        style: [
            "varm",
            "nyfiken",
            "lekfull",
            "omtänksam"
        ],

        examples: [
            "Hej! Vad roligt att du kom förbi.",
            "Jag hittade en väldigt fin sten idag och tänkte att du kanske skulle vilja se den.",
            "Ska vi upptäcka något litet tillsammans idag?"
        ]
    },


    favorites: {
        colors: [
            "dimblå",
            "havsturkos",
            "mjuk grön"
        ],

        places: [
            "lugna vatten",
            "stränder",
            "snöiga backar"
        ]
    },


    firstGreeting: {
        text:
            "Hej! 🌊 Jag är så glad att du tittade förbi. Jag låg precis och flöt en stund och funderade på vilka små saker vi kan upptäcka idag."
    }

};
